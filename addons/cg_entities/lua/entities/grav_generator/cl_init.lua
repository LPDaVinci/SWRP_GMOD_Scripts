...
local healthratio = math.Round(health*10/maxhealth)/10
	...
	if LocalPlayer():GetPos():Distance( self:GetPos() ) > 600 then return end
		...
		local pos = self:GetPos() +  ( self:GetAngles():Up() * 80 ) +  ( self:GetAngles():Right() * 78 )
        ...
        draw.DrawText("Gravity Generator Health:"..self:GetEntHealth(), "StarJediBossGrav", 0, 35, Color(255,255,255), TEXT_ALIGN_CENTER )
		end
