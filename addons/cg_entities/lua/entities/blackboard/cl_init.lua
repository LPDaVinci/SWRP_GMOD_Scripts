...
local function DrawBlackboardText()
	for _, ent in ipairs(ents.FindByClass("blackboard")) do
...
if (table.Count(msgHistory) > 0 and table.Count(userHistory) > 0) then
	...
	for k,v in pairs(msgHistory) do
		if (userHistory[k]:IsValid()) then
	    	...
	    	DermaListView:AddLine("Player disconnect",string.gsub(v,"\n","  "))
	    end
	end
end
...
BlackboardPanel:SetTitle("Blackboard")
...
BlackboardButton:SetText("Write")
